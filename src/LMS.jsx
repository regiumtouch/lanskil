...
const SKILLS = [
  ...
  { id: "seo-content-writing", name: "SEO Content Writing Masterclass", cat: "content", tier: "intermediate", lv: "Intermediate", icon: "\u{1F50D}", color: "#F4A261", dur: "10h", roles: ["Content Strategist", "SEO Specialist", "Freelance Writer"], desc: "Rank in Google AND get cited by AI Overviews, Perplexity, and ChatGPT. 7 modules covering foundations, keywords, on-page, E-E-A-T, AI search, clusters, and iteration.", lessons: [
      ...
      <div className={"sb "+(sbo?"o":"")}>
        <div style={{padding:"24px 20px 20px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
            <div style={{width:38,height:38,borderRadius:12,background:"linear-gradient(135deg,#7C3AED,#6D28D9)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:18,fontWeight:900,fontFamily:"'Cormorant Garamond',serif",transform:"rotate(-3deg)",boxShadow:"0 3px 12px rgba(124,58,237,.25)"}}>L</div>
            <div><span style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:800,fontSize:20,letterSpacing:"-0.5px",display:"block"}}>LanSkil</span><span style={{fontSize:10,color:"#AAA",fontWeight:500}}>by Regium Touch</span></div>
          </div>
        </div>
...
function Dash(props){var ft=props.ft,sr=props.sr,setSr=props.setSr,fc=props.fc,fl=props.fl,setFl=props.setFl,osk=props.osk,oskl=props.oskl,gp=props.gp,op=props.op,tl=props.tl,dc=props.dc,dn=props.dn,user=props.user,T=props.T,dark=props.dark,openTrack=props.openTrack;
  ...
  <div style={{background:"linear-gradient(135deg,#1A1A1A 0%,#2D2D2D 100%)",borderRadius:24,padding:"36px 40px",marginBottom:28,position:"relative",overflow:"hidden"}}>
    ...
    <div style={{position:"relative",zIndex:1}}>
      ...
      <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(24px,3vw,32px)",fontWeight:800,letterSpacing:"-0.8px",marginBottom:6,color:"white"}}>LanSkil Skills Library</h1>
  ...
  {continueList.length>0&&<div style={{marginBottom:28}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,lineHeight:1.2,color:T.text}}>Continue Learning</h2><p style={{fontSize:12,color:T.text3}}>Pick up where you left off</p></div>
      </div>
    </div>
        ...
        return <div key={s.id} className="c" onClick={function(){oskl(s,l);}} style={{padding:20,cursor:"pointer",borderLeft:"4px solid "+s.color,background:"linear-gradient(135deg,"+s.color+"08,"+T.card+")"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:10}}>
            <div style={{flex:1,minWidth:0}}>
              ...
              <h4 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:700,color:T.text,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.title}</h4>
  ...
  {/* Empty State */}
  {ft.length===0&&<div style={{textAlign:"center",padding:"80px 20px"}}><div style={{fontSize:56,marginBottom:16}}>{"\u{1F50D}"}</div><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:700,marginBottom:8}}>No skills found</h3><p style={{color:"#888",fontSize:15}}>Try adjusting your search or filters.</p></div>}
...
function TierBlock(props){var T=props.T,label=props.label,heading=props.heading,sublabel=props.sublabel,count=props.count,skills=props.skills,osk=props.osk,gp=props.gp,cat=props.cat,user=props.user,dn=props.dn;
  ...
  var tierComplete=activeSkills.length>0&&completedSkills.length===activeSkills.length;
  ...
  return <div style={{position:"relative",background:T.card,border:"1px solid "+T.border,borderRadius:6,padding:"32px 32px 28px",marginBottom:20,boxShadow:"0 2px 10px rgba(0,0,0,.03)"}}>
    ...
    {/* Tier certificate panel */}
    {activeSkills.length>0&&<div style={{marginTop:28,padding:"22px 26px",borderRadius:4,background:tierComplete?"linear-gradient(135deg,#F3EDFF,#FFF7ED)":T.subtle,border:"1px "+(tierComplete?"solid":"dashed")+" "+(tierComplete?tierAccent:T.border)}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:14}}>
        <div style={{flex:1,minWidth:240}}>
          <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,fontWeight:700,color:tierAccent,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>{tierComplete?"\u{1F393} Tier Certificate Unlocked":"Tier Certificate"}</div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:T.text,marginBottom:4,lineHeight:1.3}}>{tierComplete?"Congratulations \u2014 you\u2019ve completed "+heading:"Complete every course in this tier to earn the "+heading+" certificate"}</div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginTop:8}}>
            <div style={{height:4,flex:1,background:T.border,borderRadius:2,overflow:"hidden",maxWidth:220}}>
              <div style={{height:"100%",background:"linear-gradient(90deg,"+tierAccent+",#7C3AED)",width:tierProgress+"%",transition:"width .5s"}}/>
            </div>
            <span style={{fontSize:11,fontFamily:"'DM Sans',sans-serif",color:T.text3,fontWeight:600,letterSpacing:1,textTransform:"uppercase"}}>{completedSkills.length}/{activeSkills.length} Complete</span>
          </div>
        </div>
        {tierComplete&&<button className="bt" onClick={function(){printTierCert(user,label,heading,completedSkills);}} style={{padding:"12px 24px",borderRadius:3,background:"linear-gradient(135deg,#7C3AED,#F4A261)",color:"white",fontSize:12,fontWeight:700,border:"none",fontFamily:"'DM Sans',sans-serif",letterSpacing:.8,textTransform:"uppercase",cursor:"pointer",boxShadow:"0 6px 18px rgba(124,58,237,.25)"}}>{"\u{1F4DC}"} Download Tier Certificate</button>}
      </div>
    </div>}
  </div>;
}
...
function SkV(props){var sk=props.sk,gb=props.gb,ols=props.ols,dn=props.dn,gp=props.gp;var pr=gp(sk),d=sk.lessons.filter(function(l){return dn[l.id];}).length;
  ...
  <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:700,marginBottom:14}}>Lessons</h2>
...
function LBr(props){var ls=props.ls,sk=props.sk,gb=props.gb,dn=props.dn,go=props.go,goPortfolio=props.goPortfolio;var isDone=dn[ls.id];var sl=genSlides(ls);var tc=TC[ls.type]||"#666";
  return <div className="fi"><div className="bc"><span onClick={gb}>Dashboard</span><span className="sep">/</span><span onClick={gb}>{sk.name}</span><span className="sep">/</span><span className="cur">{ls.title}</span></div><div className="c" style={{padding:36,marginBottom:24,borderLeft:"4px solid "+sk.color}}><div style={{display:"flex",gap:7,marginBottom:12,flexWrap:"wrap"}}><span className="bg" style={{background:sk.color+"18",color:sk.color}}>{sk.name}</span><span className="bg" style={{background:tc+"15",color:tc}}>{ls.type}</span><span className="bg" style={{background:"#E8E8E8",color:"#777"}}>{"\u23F1"} {ls.duration}</span>{ls.portfolio&&<span className="bg" style={{background:"linear-gradient(135deg,#7C3AED,#F4A261)",color:"white",fontWeight:700}}>Portfolio Capstone</span>}</div><h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:800,marginBottom:10}}>{ls.title}</h1><p style={{fontSize:14.5,color:"#777",lineHeight:1.7,marginBottom:24}}>{ls.desc}</p>
    <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
      <button className="bt" onClick={go} style={{display:"inline-flex",alignItems:"center",gap:8,padding:"14px 30px",borderRadius:50,background:sk.color,color:"white",fontSize:14,fontWeight:700,border:"none",boxShadow:"0 4px 20px "+sk.color+"40"}}>{isDone?"\u21BB Review":"\u25B6 Start"} ({sl.length} slides + quiz)</button>
      {ls.portfolio&&<button className="bt" onClick={goPortfolio} style={{display:"inline-flex",alignItems:"center",gap:8,padding:"14px 28px",borderRadius:50,background:"linear-gradient(135deg,#7C3AED,#F4A261)",color:"white",fontSize:14,fontWeight:700,border:"none",boxShadow:"0 6px 22px rgba(124,58,237,.28)"}}>{"\u{1F4D1}"} Open Portfolio Builder</button>}
      {isDone&&<span style={{color:"#10B981",fontSize:13,fontWeight:600}}>{"\u2713"} Completed</span>}
    </div>
    <div style={{marginTop:10,fontSize:12,color:"#999"}}>{ls.portfolio?"Work through the slides, then build your portfolio submission in the Portfolio Builder.":"Complete slides then pass the quiz (67%) to finish."}</div></div>
  <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:14,fontWeight:700,marginBottom:12}}>What You'll Learn</h3><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>{(ls.topics||[]).map(function(t,i){return <div key={i} className="c" style={{padding:16,display:"flex",alignItems:"center",gap:10}}><div style={{width:32,height:32,borderRadius:8,background:sk.color+"15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>{ICO[i%ICO.length]}</div><span style={{fontSize:13,fontWeight:500}}>{t}</span></div>;})}</div></div>;}
...
function SlV(props){
  ...
  return <div className="fi">
    ...
    {isRich?
      ...
      :
      <div key={si} className="c slide-enter" style={{padding:0,overflow:"hidden",marginBottom:20,minHeight:360}}><div style={{background:"linear-gradient(135deg,"+sk.color+"15,"+sk.color+"08)",padding:"28px 36px 20px",borderBottom:"1px solid #E8E8E8"}}><div style={{fontSize:40,marginBottom:10}}>{s.icon}</div>{s.type==="intro"&&<div><span className="bg" style={{background:sk.color+"20",color:sk.color}}>{s.subtitle}</span><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:800,marginTop:6}}>{s.title}</h2></div>}{s.type==="topic"&&<div><span className="bg" style={{background:"#E8E8E8",color:"#777"}}>Topic {s.num} of {s.total}</span><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:800,marginTop:6}}>{s.title}</h2></div>}{s.type==="summary"&&<h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:800}}>Key Takeaways</h2>}</div>
  ...
  return <div className="fi" style={{maxWidth:640,margin:"40px auto 0"}}>
    <div className="c" style={{padding:"48px 40px",textAlign:"center",borderTop:"4px solid "+sk.color,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:800,marginBottom:10,color:T.text}}>Slides Complete!</h1>
  ...
  if(qs)return <div className="fi">
    <div className="c" style={{padding:"40px 36px",borderTop:"4px solid "+(pass?"#10B981":"#EF4444"),overflow:"hidden"}}>
      <div style={{textAlign:"center",marginBottom:28}}>
        ...
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:800,marginBottom:6}}>{pass?"Quiz Passed!":"Not quite!"}</h2>
      ...
      <div style={{marginBottom:24}}>
        ...
        {qz.map(function(q2,i){var isCorrect=qa[i]===q2.correct;return <div key={i} className={"quiz-review "+(isCorrect?"quiz-correct":"quiz-wrong")}>
          ...
          <div style={{flex:1}}><p style={{fontSize:13,fontWeight:600,color:"#1A1A1A",marginBottom:4}}>{q2.q}</p>
            {!isCorrect&&<p style={{fontSize:12,color:"#DC2626",marginBottom:2}}>Your answer: {qa[i]<q2.opts.length?q2.opts[qa[i]]:["None of the above","All of the above","I don't know"][qa[i]-q2.opts.length]}</p>}
...
function printTierCert(user,tierLabel,tierHeading,skills){var w=window.open("","_blank","width=900,height=650");if(!w)return;var name=(user?user.firstName+" "+(user.lastName||""):"Learner").trim();var date=new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"});var skillRows=skills.map(function(s){return '<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px dashed #E5E5E5;"><span style="font-family:\'Cormorant Garamond\',serif;font-size:15px;color:#1A1A1A;font-weight:600">'+s.name+'</span><span style="font-family:\'DM Sans\',sans-serif;font-size:11px;color:#F4A261;letter-spacing:1.5px;text-transform:uppercase;font-weight:700">Completed</span></div>';}).join("");w.document.write('<!DOCTYPE html><html><head><title>LanSkil '+tierLabel+' Certificate</title><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500;1,600;1,700&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{display:flex;align-items:center;justify-content:center;min-height:100vh;background:#F5F5F5;font-family:"DM Sans",sans-serif;padding:20px}@media print{body{background:white;padding:0}.no-print{display:none!important}}.cert{width:840px;padding:56px 64px;background:white;border:3px solid #7C3AED;position:relative;box-shadow:0 8px 40px rgba(0,0,0,.1)}.cert::before{content:"";position:absolute;inset:10px;border:1.5px solid #F4A261;pointer-events:none}.header{display:flex;align-items:center;gap:14px;justify-content:center;margin-bottom:6px}.mark{width:44px;height:44px;background:linear-gradient(135deg,#7C3AED,#F4A261);border-radius:10px;display:flex;align-items:center;justify-content:center;color:white;font-size:22px;font-weight:900;font-family:"Cormorant Garamond",serif;transform:rotate(-3deg)}.brand{font-family:"Cormorant Garamond",serif;font-weight:700;font-size:28px;color:#1A1A1A}.sub{text-align:center;color:#AAA;font-size:11px;margin-bottom:36px;letter-spacing:2px}.tier{text-align:center;font-family:"Cormorant Garamond",serif;font-style:italic;font-size:14px;color:#F4A261;letter-spacing:3px;text-transform:uppercase;font-weight:600;margin-bottom:6px}.title{text-align:center;font-family:"Cormorant Garamond",serif;font-weight:700;font-size:32px;color:#1A1A1A;margin-bottom:16px;letter-spacing:-.5px}.awarded{text-align:center;font-size:13px;color:#777;letter-spacing:1.5px;text-transform:uppercase;font-weight:600;margin-bottom:12px}.name{text-align:center;font-family:"Cormorant Garamond",serif;font-size:44px;font-weight:700;color:#1A1A1A;margin-bottom:22px;line-height:1.15;letter-spacing:-.5px}.desc{text-align:center;font-size:13px;color:#666;line-height:1.7;max-width:560px;margin:0 auto 28px;font-family:"DM Sans",sans-serif}.completed-label{text-align:center;font-family:"DM Sans",sans-serif;font-size:10px;color:#7C3AED;letter-spacing:2.5px;text-transform:uppercase;font-weight:700;margin-bottom:14px}.skills-box{background:linear-gradient(135deg,#F3EDFF22,#FFF7ED33);border:1px solid #7C3AED22;border-radius:4px;padding:16px 22px;margin-bottom:30px}.footer{display:flex;justify-content:space-between;align-items:flex-end;padding-top:24px;border-top:1px solid #EEE}.sig{text-align:center}.sig-line{width:180px;border-top:1.5px solid #CCC;margin-bottom:6px}.sig-name{font-size:12px;color:#999;font-family:"DM Sans",sans-serif}.sig-line-name{font-family:"Cormorant Garamond",serif;font-style:italic;font-size:16px;color:#333;margin-bottom:2px}.actions{text-align:center;margin-top:20px}.actions button{padding:12px 32px;border-radius:50px;border:none;background:#7C3AED;color:white;font-size:14px;font-weight:700;cursor:pointer;font-family:"DM Sans",sans-serif;margin:0 8px}</style></head><body><div class="cert"><div class="header"><div class="mark">L</div><div class="brand">LanSkil</div></div><div class="sub">BY REGIUM TOUCH</div><div class="tier">'+tierLabel+'</div><div class="title">'+tierHeading+'</div><div class="awarded">Certificate of Completion</div><div class="name">'+name+'</div><div class="desc">has successfully completed every required course in this tier of the LanSkil Copywriting curriculum, demonstrating mastery of the full tier syllabus.</div><div class="completed-label">Courses Completed</div><div class="skills-box">'+skillRows+'</div><div class="footer"><div class="sig"><div class="sig-line-name">Regium Touch</div><div class="sig-line"></div><div class="sig-name">Program Director</div></div><div class="sig"><div class="sig-line-name">'+date+'</div><div class="sig-line"></div><div class="sig-name">Date Awarded</div></div></div></div><div class="actions no-print"><button onclick="window.print()">Print / Save as PDF</button><button onclick="window.close()" style="background:#888">Close</button></div></body></html>');w.document.close();}

function printCert(user,sk){var w=window.open("","_blank","width=900,height=650");if(!w)return;var name=(user?user.firstName+" "+(user.lastName||""):"Learner").trim();var date=new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"});w.document.write('<!DOCTYPE html><html><head><title>Certificate - '+sk.name+'</title><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500;1,600;1,700&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{display:flex;align-items:center;justify-content:center;min-height:100vh;background:#F5F5F5;font-family:"DM Sans",sans-serif}@media print{body{background:white}.no-print{display:none!important}}.cert{width:800px;padding:60px;background:white;border:3px solid #7C3AED;position:relative;box-shadow:0 8px 40px rgba(0,0,0,.1)}.cert::before{content:"";position:absolute;inset:8px;border:1.5px solid #F4A261;pointer-events:none}.logo{display:flex;align-items:center;gap:10px;justify-content:center;margin-bottom:8px}.logo-mark{width:40px;height:40px;background:#7C3AED;border-radius:10px;display:flex;align-items:center;justify-content:center;color:white;font-size:20px;font-weight:900;font-family:"Cormorant Garamond",serif;transform:rotate(-3deg)}.logo-text{font-family:"Cormorant Garamond",serif;font-weight:900;font-size:24px}.sub{text-align:center;color:#AAA;font-size:12px;margin-bottom:32px}.title{text-align:center;font-size:14px;color:#999;text-transform:uppercase;letter-spacing:3px;margin-bottom:12px}.name{text-align:center;font-family:"Cormorant Garamond",serif;font-size:42px;font-weight:800;color:#1A1A1A;margin-bottom:8px;line-height:1.2}.course{text-align:center;font-size:16px;color:#555;margin-bottom:6px}.skill{text-align:center;font-family:"Cormorant Garamond",serif;font-size:24px;font-weight:700;color:#7C3AED;margin-bottom:24px}.date{text-align:center;font-size:14px;color:#999;margin-bottom:32px}.footer{display:flex;justify-content:space-between;align-items:flex-end;padding-top:24px;border-top:1px solid #EEE}.sig{text-align:center}.sig-line{width:160px;border-top:1.5px solid #CCC;margin-bottom:6px}.sig-name{font-size:12px;color:#999}.actions{text-align:center;margin-top:20px}.actions button{padding:12px 32px;border-radius:50px;border:none;background:#7C3AED;color:white;font-size:14px;font-weight:700;cursor:pointer;font-family:"DM Sans",sans-serif;margin:0 8px}</style></head><body><div class="cert"><div class="logo"><div class="logo-mark">L</div><div class="logo-text">LanSkil</div></div><div class="sub">by Regium Touch</div><div class="title">Certificate of Completion</div><div class="name">'+name+'</div><div class="course">has successfully completed</div><div class="skill">'+sk.name+'</div><div class="date">'+date+'</div><div class="footer"><div class="sig"><div class="sig-line"></div><div class="sig-name">Program Director</div></div><div class="sig"><div class="sig-line"></div><div class="sig-name">'+date+'</div></div></div></div><div class="actions no-print"><button onclick="window.print()">Print / Save as PDF</button><button onclick="window.close()" style="background:#888">Close</button></div></body></html>');w.document.close();}
  ...
  return <div className="fi" style={{textAlign:"center",paddingTop:40}}>
    ...
    <div className="c" style={{padding:"52px 48px",maxWidth:540,margin:"0 auto",borderTop:"4px solid #10B981",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:800,marginBottom:10}}>{allDone?"Skill Complete!":"Lesson Complete!"}</h1>
        ...
        {qScore&&<div style={{marginBottom:24,padding:"24px 20px",borderRadius:20,background:"linear-gradient(135deg,"+sk.color+"15,"+sk.color+"08)",border:"1px solid "+sk.color+"30"}}>
          ...
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:52,fontWeight:900,color:sk.color,lineHeight:1}}>{qScore.pct}%</div>
        ...
        {allDone&&<div style={{marginBottom:24}}>
          <div style={{padding:"18px 28px",borderRadius:16,background:"linear-gradient(135deg,#F0FDF4,#ECFDF5)",border:"1px solid #BBF7D0",marginBottom:16}}><p style={{color:"#15803D",fontSize:15,fontWeight:700,marginBottom:4}}>{"\u{1F389}"} Course Complete!</p><p style={{color:"#059669",fontSize:13}}>You\u2019ve mastered {sk.name}</p></div>
          {sk.cat==="content"
            ? <div style={{padding:"16px 22px",borderRadius:12,background:"linear-gradient(135deg,#F3EDFF,#FFF7ED)",border:"1px dashed #7C3AED55"}}>
                <p style={{color:"#6D28D9",fontSize:13,fontWeight:700,marginBottom:4,letterSpacing:.3}}>{"\u{1F393}"} Certificates are awarded per tier</p>
                <p style={{color:"#555",fontSize:12.5,lineHeight:1.55}}>Finish every course in this tier to unlock your tier certificate. You can download it from the tier card on the dashboard.</p>
              </div>
